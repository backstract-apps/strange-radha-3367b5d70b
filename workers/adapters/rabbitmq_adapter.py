    ...
    async def subscribe(
        ...
        try:
            ...
            async def message_handler(message: aio_pika.IncomingMessage):
                ack_on_error = config.get("ack_on_error", True) if config else True
                requeue_on_error = config.get("requeue_on_error", False) if config else False
                async with message.process(requeue=requeue_on_error):
                    ...
                    try:
                        ...
                        logger.error(
                            f"Failed job from {queue_name}; ack_on_error={ack_on_error}; "
                            f"requeue_on_error={requeue_on_error}; error={e}; payload={body}"
                        )
                        if not ack_on_error:
                            raise
